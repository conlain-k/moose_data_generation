...
group.add_argument(
    "--ttv_split",
    help="Split a dataset into train-test-validate sets. Takes four args: <micro_file>, <resp_file>, <train relative fraction>, <test relative fraction>. Deduces validation size from other two. Relative fractions must be between zero and 1, and sum to 1 at most.",
    nargs=4,
)
...
def concat_files(basename, output_file, entries_per_file=1):
    allDats = glob.glob(basename + "/*_stress_strain.h5")
        ...
        for key in data_i.keys():
            ...
            if curr_dataset is not None:
                ...
                if big_file.get(key) is None:
                    ...
                    if len(curr_dataset.shape) == 4:
                        chunk_size = (1,) + curr_dataset.shape[:]
                        newshape = (max_num_ents,) + curr_dataset.shape[:]
                ...
                big_file[key][start_inds[key] : start_inds[key] + entries] = (
                    curr_dataset[:]
                )
...
def ttv_split(micro_file, resp_file, train_frac, test_frac):
    ...
    def write_split(name, hfile, inds, stype):
        ...
        for k in hfile.keys():
            ...
            hfile_new.create_dataset(
                k,
                data=hfile[k][inds],
                shuffle=True,
                compression="gzip",
                compression_opts=4,
                chunks=hfile[k].chunks,
            )
    ...
    valid_frac = 1.0 - train_frac - test_frac
    check_frac(train_frac)
